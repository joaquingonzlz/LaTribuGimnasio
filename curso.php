...
if(!esParticipante($_SESSION['user'], getCourse($_GET['course']))) http_response_code(403);
	...
	if(!esProfesor($_SESSION['user'])){
		...
		if($ultimo_visto == null) $ultimo_visto = $db->query("SELECT video FROM clases WHERE curso = $curso LIMIT 1")->fetchColumn();
		...
		// $ultimo_visto = $db->query("SELECT video FROM clases WHERE curso = $curso LIMIT 1")->fetchColumn();
	}
